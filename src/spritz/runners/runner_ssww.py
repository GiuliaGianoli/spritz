...
def process(events, **kwargs):
    ...
    if not isData:
        ...
        if do_theory_variations:
            print("Applying theory variations (SSWW)")
    ...
    #fake rate
    if isFake:
        print("Applying fake rate (SSWW)")
        events, variations = fake_evaluate_ssww_syst(events, variations, ceval_fake, cfg)
    ...
    for variation in sorted(variations.get_variations_all()):
        ...
        if not isFake:
            ...
            for ilep in range(2):
                ...
                # comb = comb & (
                #     events.Lepton[:, ilep][eleWP]
                #     | events.Lepton[:, ilep][muWP]
                # )
        ...
        # leptoncut = (
        # leptoncut & (events.Lepton[:, 0].pt > 10) & (events.Lepton[:, 1].pt > 10)
        # )
