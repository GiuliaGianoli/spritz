...
from spritz.modules.fake_evaluate_wz_syst import fake_evaluate_wz_syst
from spritz.modules.fake_evaluate_wz import fake_evaluate_wz
...
fake_json_dir = cfg["fakeRate"]  # cfg.json

for fname in os.listdir(fake_json_dir):
    if not fname.endswith(".json"):
        continue
    path = os.path.join(fake_json_dir, fname)
    cset = correctionlib.CorrectionSet.from_file(path)
    for key in cset.keys():
        ceval_fake[key] = cset[key]
...
def process(events, **kwargs):
    ...
    isFake = kwargs.get("is_fake", False)
    ...
    #fake rate
    if isFake:
        print("Applying fake rate")
        events, variations = fake_evaluate_wz_syst(events, variations, ceval_fake, cfg)
        #events = fake_evaluate_wz(events, ceval_fake, cfg)
    ...
    syst_labels = sorted(set(variations.get_variations_all()) | {"FW_up", "FW_down"})

    default_axis = [
        hist.axis.StrCategory([region for region in regions], name="category"),
        hist.axis.StrCategory(syst_labels, name="syst"),
    ]
    ...
    for variation in sorted(variations.get_variations_all()):
        ...
        # l3tight (non ai fake) per WZ region
        if not isFake:
            events = events[(ak.num(events.Lepton, axis=1) >= 3)]
            ...
            events["l3Tight"] = ak.copy(comb)
            events = events[events.l3Tight]
        ...
        if not isData:
            events["prompt_gen_match_2l"] = (
                ...
                & ak.fill_none(ak.pad_none(events.Lepton, 3)[:, 2].promptgenmatched, False)
            )
        ...
        if not isData:
            ...
            events["RecoSF"] = events.Lepton[:, 0].RecoSF * events.Lepton[:, 1].RecoSF * events.Lepton[:, 2].RecoSF
            events["TightSF"] = (
                events.Lepton[:, 0].TightSF * events.Lepton[:, 1].TightSF * events.Lepton[:, 2].TightSF
            )
        ...
        if isFake and "fakeWeight" in events.fields:
            print("Applying fake weight")
            events["weight"] = events.weight*events.fakeWeight
